...
const double TEMPCOEFF = 0.0000001;
...
const double CROSS = 6.0E-22 / M_SCALE / M_SCALE;//proton cross section in code units
...
const double DOMEGA = 4.0*PI/(PHIRES*SINTHETARES);
...
{
  int i;
  i = floor((phi/(2.0*PI))*PHIRES);
  if (i < round(PHIRES)) return i;
  else return round(PHIRES)-1;
      
}

int getrindex(double r)
{
  return floor((r-RMIN)/(RMAX-RMIN)*RRES);
}

double phifromi(int i)
{
  //calculate phi from an index
  return (double) i/PHIRES * 2.0 * PI;
}

double thetafromj(int j)
{
  double temp;
  temp = j * 2.0/SINTHETARES - 1.0;
  return asin(temp);
}
...
{
  double temp;
  temp = k * (RMAX-RMIN) / RRES + RMIN;
  return temp;
}

void fillarrays(double ***dens, double ***velocity, char* infile, char* bhfile, double phi, double theta, double *bhmass)
{
  FILE *f,*fbh;
  int i,j,k,id,accept,discard;
  double tempx,tempy,tempz;
  char line[160];
  double tpos[3],tv[3],rotate[3],columndens;
  double bhpos[3],bhv[3],tempr,tempphi,temptheta,mass;
  ...
  fgets(line,160,fbh);
  sscanf(line, "%d %lg %lg %lg %lg %lg %lg %lg\n",&id,bhmass,&bhpos[0],&bhpos[1],&bhpos[2],&bhv[0],&bhv[1],&bhv[2]);
  ...
  while (fgets(line, 160, f) != NULL)
    {
      ...
      tempphi = atan2(tempy,tempx)+PI-phi;
      temptheta = asin(tempz/tempr)+PI/2.0-theta;//don't change if theta = pi/2 which means i look from the top
...
void newloopthroughgrid(double ***dens, double ***velocity, double mass, double clight, double phi, double theta, char* infile)
{
  /* density grid is now in spherical coordinates. loop through phi and theta and check for each r, whether all the photons are going to be absorped already*/
  double **spectrum,t_min,base_ts,strength0,r,absorbed;
  double rij_eye,lambda,dlambda,temperature,time,tcoeff,ts,difflambda;
  int i,j,k,l,accepted,rejected,intstarttime,freqbin,ll,time0;
  FILE *f,*fout;
  // double area,absorption;
  ...
  t_min = 0.11*sqrt(mass*MSUN_SCALE/(1.0E+06))*365.0;
  printf("T_min = %e days\n",t_min);
  intstarttime = floor(t_min);
  //  if i did everything correctly, each (phi,theta) ray occupies the same dOmega, so each gets the same amount of photons
  double found_gas = 0.0;
  for (i = 0; i < PHIRES; i++)
    for (j = 0; j < SINTHETARES; j++)
      {
	//calculate the absorption coefficient for the current region, if it is > 1 absorb maximum possible
	//if it is < 1 absorb only that part of it.
	absorbed = 0.0;
	for (k = 0; k < RRES; k++)
	  {
	    if (dens[i][j][k] > 0.0)
		...
		//now make the assumption that everything here is radiated to the eye directly
		r = (rfromk(k)+rfromk(k+1))/2.0;
		strength0 = 1/r/r/(4.0*PI)*DOMEGA;//geometric dilution of source luminosity
		rij_eye = cos(thetafromj(j))*r;
		//calculate the light travel time for both BHs
		time = (r+rij_eye)/clight*TIMEINDAYS + OFFSET; //when the TD reaches the gas particle and then the radiation reaches the eye
		//	  printf("%f\t%f\n",time,time2);
		lambda = baselambda/(1.0-velocity[i][j][k]/clight);
		
		temperature = TEMPCOEFF/r/r;
		dlambda = lambda*sqrt(temperature); // the broadening
		...
		for (l = 0; l < NT; l++)
		  {
		    //i use here the start point of the tidal disruption as t = 0!
		    base_ts = strength0*luminosity((double) (l+intstarttime),t_min)/PROTONMASS*HV0/TIMEINSECONDS;//this gives ergs per sec (luminosity)
		    //calculate the flux at some distance given in cm now all in physical units
		    base_ts = base_ts/(4.0*PI*DL*DL);
			...
			tcoeff = 1.0/dlambda*exp(-(difflambda*difflambda)/(dlambda*dlambda));

			ts = base_ts*tcoeff;

			// printf("diff lambda: %e strength coeff: %e temp: %e  dlambda: %e\n ",difflambda,tcoeff1,temperature1,dlambda1);
			if (((time0 + l) < DURATION) && ((time0 + l) >= 0))
			  spectrum[time0 + l][ll] += ts;
		      }
		  }
		// area = r*r*DOMEGA;
		// absorption = dens[i][j][k]/PROTONMASS*CROSS/area;
		// if (absorption > 1.0) absorbed += 1.0;
		//		    printf("Area: %e, Absorption coefficient: %e\n", area,absorption);
		break;
	      }
	  }
      }
  ...
  printf("Found Gas in %.2f%% of the sky!\n",found_gas/(4.0*PI)*100.0);
}
...
int main (int argc, char** argv)
{
  ...
  infile = argv[1];
  infileBH = argv[2];
  fillarrays(dens,velocity,infile,infileBH,phi,theta,&mass);
  newloopthroughgrid(dens,velocity,mass,clight,phi,theta,infileBH);
